    ...
    with tf.Session(config=config) as sess:
            ...
            try:
                ...
                while True:
                    ...
                    z=np.ones(shape=(M*pitch),dtype=float)*8+np.random.normal(size=M*pitch)*0.01
                    sess.run(opt_d,feed_dict={x_node:x,z_node:np.reshape(z,(pitch,M))})
                    if(int_num%100==0):
                      z=np.ones(shape=(M*pitch),dtype=float)*8+np.random.normal(size=M*pitch)*0.01
                      sess.run(opt_g,feed_dict={z_node:np.reshape(z,(pitch,M))})
                    int_num=int_num+1
                    if(int_num>200000):
                        break
