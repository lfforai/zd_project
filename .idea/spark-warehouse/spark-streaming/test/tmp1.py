...
def normal_probability(y,n=10000,p=0.95,gpu_num="0"):
    ...
    with tf.Session(config=config) as sess:
        ...
        #赋值
        value_big=tf.assign(value_big,tf.convert_to_tensor(y.max()+2,dtype=tf.float32))
        print("y.max",y.max())
        value_little=tf.assign(value_little,tf.convert_to_tensor(y.min()-2,dtype=tf.float32))
