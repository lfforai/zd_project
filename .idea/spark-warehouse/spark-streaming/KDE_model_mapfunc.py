...
def normal_probability(y,n=10000,p=0.95,gpu_num="0"):
    ...
    while True:
        with tf.Session(config=config) as sess:
            ...
            rdd_dataset=tf.data.Dataset.from_tensor_slices(x1).map(normal_probability_density(y_in,dx/2),num_parallel_calls=4).map(lambda x2:x2*dx,num_parallel_calls=4)
